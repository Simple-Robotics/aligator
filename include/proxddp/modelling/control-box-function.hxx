...
namespace proxddp
{
  ...
  template<typename Scalar>
  ControlBoxFunctionTpl<Scalar>::
  ControlBoxFunctionTpl(const int ndx, const int nu, Scalar umin, Scalar umax)
    : ControlBoxFunctionTpl(ndx, VectorXs::Constant(nu, umin),
                            VectorXs::Constant(nu, umax)) {}

  template<typename Scalar>
  void ControlBoxFunctionTpl<Scalar>::
  evaluate(const ConstVectorRef&,
  ...
  template<typename Scalar>
  void ControlBoxFunctionTpl<Scalar>::
  computeJacobians(const ConstVectorRef&,
                   const ConstVectorRef&,
                   const ConstVectorRef&,
                   Data& data) const
  {
