...
/// @brief Compute the directional derivative of the cost function.
...
///    \right\|^2
/// \f]
/// where \f$(*)\f$ is the expression within the norm in \f$\mathscr{P}\f$
/// above.
///
...
template <typename _Scalar> struct ALFunction {
  ...
  static Scalar evaluate(const Scalar mudyn, const Scalar mucstr,
                         const TrajOptProblem &problem, Workspace &workspace);
