...
struct CrocCostModelWrapperTpl : CostAbstractTpl<_Scalar> {
  ...
  std::shared_ptr<CrocCostModel> croc_cost_;
  std::shared_ptr<CrocActionModel> action_model_;

  /// Constructor from a crocoddyl cost model.
  explicit CrocCostModelWrapperTpl(std::shared_ptr<CrocCostModel> cost)
      : Base(StateWrap(cost->get_state()), (int)cost->get_nu()),
        croc_cost_(cost) {}

  /// Constructor using a terminal action model.
  explicit CrocCostModelWrapperTpl(
      std::shared_ptr<CrocActionModel> action_model)
  ...
  shared_ptr<BaseData> createData() const {
    if (action_model_ != 0) {
      std::shared_ptr<crocoddyl::ActionDataAbstractTpl<Scalar>> am_data =
...
struct CrocCostDataWrapperTpl : CostDataAbstractTpl<Scalar> {
  ...
  std::shared_ptr<CostData> croc_cost_data_;
  std::shared_ptr<ActionData> croc_act_data_;

  explicit CrocCostDataWrapperTpl(const std::shared_ptr<CostData> &crocdata)
      : Base((int)crocdata->Lx.rows(), (int)crocdata->Lu.rows()),
        croc_cost_data_(crocdata) {}

  explicit CrocCostDataWrapperTpl(const std::shared_ptr<ActionData> &actdata)
