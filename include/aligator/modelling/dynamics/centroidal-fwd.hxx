...
void CentroidalFwdDynamicsTpl<Scalar>::forward(const ConstVectorRef &x,
  ...
  d.xdot_.segment(3, 3) = gravity_;
  d.xdot_.segment(6, 3) << 0, 0, 0;
  for (std::size_t i = 0; i < nk_; i++) {
    if (active_contacts_[i]) {
      d.xdot_.segment(3, 3) += u.segment(i * 6, 3);
      d.xdot_.segment(6, 3) += u.segment(i * 6 + 3, 3);
...
void CentroidalFwdDynamicsTpl<Scalar>::dForward(const ConstVectorRef &x,
  ...
  for (std::size_t i = 0; i < nk_; i++) {
    if (active_contacts_[i]) {
      d.Ju_.block(6, 6 * i, 3, 3) << 0.0, -(contact_points_[i][2] - x[2]),
      ...
      d.Ju_.block(3, 6 * i, 3, 3) = Matrix3s::Identity();
      d.Ju_.block(6, 6 * i + 3, 3, 3) = Matrix3s::Identity();
    }
  }
}
