...
cvg = solver.solve(xs_init, us_init, max_iters, False, reg_init)
...
solver2 = proxddp.SolverFDDP(tol / nsteps)
...
x_err_v = [
    np.max(abs(solver.xs[t] - rs.xs[t]))
    for t in range(nsteps + 1)
]
print("xerr = {}".format(max(x_err_v)))
u_err_v = [
    np.max(abs(solver.us[t] - rs.us[t]))
