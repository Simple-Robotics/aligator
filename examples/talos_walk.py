...
def create_dynamics(support):
    dyn_model = None
    if support=="LEFT":
        ode = dynamics.MultibodyConstraintFwdDynamics(space, act_matrix, [constraint_models[0]], prox_settings)
        dyn_model = dynamics.IntegratorSemiImplEuler(ode, dt)
    elif support=="RIGHT":
        ode = dynamics.MultibodyConstraintFwdDynamics(space, act_matrix, [constraint_models[1]], prox_settings)
        dyn_model = dynamics.IntegratorSemiImplEuler(ode, dt)
    else:
        ode = dynamics.MultibodyConstraintFwdDynamics(space, act_matrix, constraint_models, prox_settings)
        dyn_model = dynamics.IntegratorSemiImplEuler(ode, dt)
    return dyn_model 
...
v_ref = pin.Motion()
v_ref.np[:] = 0.0
frame_vel_LF = aligator.FrameVelocityResidual(space.ndx, nu, rmodel, v_ref, LF_id, pin.LOCAL)
frame_vel_RF = aligator.FrameVelocityResidual(space.ndx, nu, rmodel, v_ref, RF_id, pin.LOCAL)
...
def createStage(support, prev_support, LF_target, RF_target):
    ...
    frame_cs_RF = aligator.FrameTranslationResidual(
        space.ndx, nu, rmodel, RF_target.translation, RF_id)[2]
    frame_cs_LF = aligator.FrameTranslationResidual(
        space.ndx, nu, rmodel, LF_target.translation, LF_id)[2]
...
for i in range(1,nsteps):
    stages.append(createStage(contact_phases[i],contact_phases[i-1], LF_placements[i], RF_placements[i]))
...
#solver.sa_mode = aligator.FILTER
...
for i, cp in enumerate(contact_phases):
    if cp == "LEFT":
        force_left.append(workspace.problem_data.stage_data[i].constraint_data[0].continuous_data.constraint_datas[0].contact_force.linear)
        force_right.append(np.zeros(3))
    elif cp == "RIGHT":
        force_right.append(workspace.problem_data.stage_data[i].constraint_data[0].continuous_data.constraint_datas[0].contact_force.linear)
        force_left.append(np.zeros(3))
    else:
        force_left.append(workspace.problem_data.stage_data[i].constraint_data[0].continuous_data.constraint_datas[0].contact_force.linear)
        force_right.append(workspace.problem_data.stage_data[i].constraint_data[0].continuous_data.constraint_datas[1].contact_force.linear)
